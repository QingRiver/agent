...
describe('resolveEntityRef', () => {
  ...
  it('rEF_NOT_FOUND', () => {
    ...
    if (!r.ok)
      expect(r.error.code).toBe(FILTER_ERROR_CODE.REF_NOT_FOUND)
  })
  ...
  it('aMBIGUOUS_REF', () => {
    ...
    expect(r.ok).toBe(false)
    if (!r.ok)
      expect(r.error.code).toBe(FILTER_ERROR_CODE.AMBIGUOUS_REF)
  })
  ...
  it('rEF_CONFLICT', () => {
    ...
    expect(r.ok).toBe(false)
    if (!r.ok)
      expect(r.error.code).toBe(FILTER_ERROR_CODE.REF_CONFLICT)
  })
})
...
describe('validatePerspectiveInput', () => {
  ...
  it('persist 拒绝相对日期', () => {
    ...
    if (!result.ok) {
      const code = FILTER_ERROR_CODE.INVALID_DATE_TOKEN
  ...
  it('eMPTY_NAME', () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok)
      expect(result.errors[0]?.code).toBe(FILTER_ERROR_CODE.EMPTY_NAME)
  })
  ...
  it('iNVALID_FIELD_OP', () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok)
      expect(result.errors[0]?.code).toBe(FILTER_ERROR_CODE.INVALID_FIELD_OP)
  })
  ...
  it('iNVALID_DATE_RANGE within 倒置', () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok)
      expect(result.errors[0]?.code).toBe(FILTER_ERROR_CODE.INVALID_DATE_RANGE)
  })
  ...
  it('dUPLICATE_SORT_KEY', () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok)
      expect(result.errors[0]?.code).toBe(FILTER_ERROR_CODE.DUPLICATE_SORT_KEY)
  })
