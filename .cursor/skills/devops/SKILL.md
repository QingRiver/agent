...
name: devops
description: >-
  本仓库基础设施与 E2E 的统一入口。用户或 agent 要启动/检查 Docker infra（qdrant、markitdown、qlib）、
...
**唯一 CLI**：`pnpm devops <command> ...`（实现：`.cursor/skills/devops/scripts/cli.ts`）
...
`status` 会检查 Docker 容器是否 running，并对 HTTP `/health(z)` 探活。

## e2e

测试账号（`pnpm devops e2e auth` 写入 `auth.sqlite`）：

- 邮箱：`agent-e2e@cursor.local`
- 密码：`agent-e2e-pass`

```bash
pnpm devops e2e all          # auth seed → kb seed → kb vitest → hitl vitest（不需 server）
pnpm devops e2e seed         # 仅种子数据
