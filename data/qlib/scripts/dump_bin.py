...
class DumpDataBase:
    ...
    def _parse_date_series(series: pd.Series) -> pd.Series:
        ...
        if as_str.str.fullmatch(r"\d{8}").all():
            return pd.to_datetime(as_str, format="%Y%m%d")
        return pd.to_datetime(series)

    def _format_datetime(self, datetime_d: [str, pd.Timestamp]):
        if isinstance(datetime_d, (int, np.integer)):
            datetime_d = pd.to_datetime(str(datetime_d), format="%Y%m%d")
        else:
    ...
    def _get_source_data(self, file_path: Path) -> pd.DataFrame:
        ...
        if self.date_field_name in df.columns:
            df[self.date_field_name] = self._parse_date_series(df[self.date_field_name])
    ...
    def _load_all_source_data(self):
        ...
        def _read_df(file_path: Path):
            ...
            if self.date_field_name in _df.columns and not np.issubdtype(
                ...
                _df[self.date_field_name] = self._parse_date_series(_df[self.date_field_name])
