...
| `PORT` | 监听端口（可在 `apps/server/gateway/.env` 覆盖） |
| `RSC_ENGINE_URL` | rsc-engine loopback（默认 `http://127.0.0.1:3010`）；`POST /rsc/render` 透传 |
...
pnpm --filter rsc-engine dev      # Waku RSC 引擎（根 pnpm dev 已并行启动）
```
